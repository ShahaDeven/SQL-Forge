...
"Self-hosting is 54× cheaper" is the headline every writeup like this reaches for, and on
its own it's misleading — it's true only when the GPU is saturated. A rented GPU bills by
the hour whether or not it's serving anything; the API is pure per-token with zero idle
cost. So the useful number isn't a ratio, it's a **break-even**.

Every input below is measured, not assumed: throughput from the eval's recorded wall-clock,
and Claude token counts from Anthropic's `count_tokens` endpoint on the *identical* prompts.

| Option | tokens in/out | $ / 1k queries | vs Sonnet 5 |
|---|---:|---:|---:|
| **SFT-7B self-hosted** (74.5%) | — / 107 | **$0.12** | 54× cheaper |
| **SFT-1.5B self-hosted** (70.9%) | — / 103 | **$0.05** | 136× cheaper |
| Claude Haiku 4.5 | 878 / 102 | $1.39 | 4.7× cheaper |
| Claude Sonnet 5 (intro rate) | 1394 / 157 | $4.36 | 1.5× cheaper |
| Claude Sonnet 5 (list) | 1394 / 157 | $6.54 | — |

*Self-hosted figures assume a saturated A40 at $0.79/hr (RunPod Secure Cloud list, 2026-07)
— the best case. API prices are list, 2026-07. Generated by `sqlforge/eval/cost_analysis.py`.*

### The break-even

Break-even is `GPU $/hour ÷ API $/query` — it depends only on those two numbers, **not** on
how fast the local model is. Throughput sets *capacity*, not the crossover:

| | Break-even | GPU capacity (7B) | Utilization at break-even |
|---|---:|---:|---:|
| vs Claude Sonnet 5 | **2,900 queries/day** (121/hr) | 156,470/day | **1.9%** |
| vs Claude Haiku 4.5 | 13,687 queries/day (570/hr) | 156,470/day | 8.7% |

**Below ~2,900 queries/day, the API is cheaper** — the GPU sits idle burning money. Above it,
self-hosting wins, and the margin widens with volume. The striking part is how little of the
GPU that takes: break-even against Sonnet 5 needs **1.9% utilization**. The remaining 98% of
capacity is where the 54× lives.

### A tokenizer gotcha worth knowing

**Claude's tokenizer is not Qwen's.** The same prompt counts **743.6** tokens for Qwen and
**1,393.9** for Sonnet 5 — ~1.9×, because Sonnet 5 ships a new tokenizer. Pricing the API
off locally-measured token counts would understate its cost by ~47%. The counts here come
from `count_tokens` per model (cached in `sqlforge/eval/token_counts.json`).

### What this comparison does *not* include

- **Claude's 91.7% is the full agent** — retrieval, few-shot examples, and up to 3
  self-healing retries. Its real per-query cost is therefore **higher** than the single-shot
  figure above (more input tokens, sometimes 3× the calls). The cost gap is understated; the
  accuracy gap is not like-for-like either.
- **Engineering cost is excluded.** The 7B adapter took ~2 days and ~$15 of GPU + API spend
  to produce, plus the ops burden of running the pod. At 2,900 queries/day you break even on
  *inference* and still haven't paid back the build.
- One provider, one GPU, one point in time — pass `--gpu-rate` to re-run against your own.
